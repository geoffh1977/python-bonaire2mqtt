...
from constants import (
    HVAC_MODE_OFF, HVAC_MODE_HEAT, HVAC_MODE_COOL, HVAC_MODE_FAN_ONLY, MAX_TEMP, MIN_TEMP, HEAT_FAN_MODES, COOL_FAN_MODES,
    DELETE, DISCOVERY, EVAP_FAN_MODES, EVAP_MAX_TEMP, EVAP_MIN_TEMP, FAN_ONLY_FAN_MODES, GETZONEINFO, INSTALLATION, LOCAL_PORT,
    POSTZONEINFO, POSTZONEINFOFAN, UDP_DISCOVERY_PORT)
...
class HandleUDPBroadcast:
    ...
    def connection_made(self, transport):
        _LOGGER.info("Sending Bonaire discovery message...")
...
class BonairePyClimate():
    ...
    def turn_zone_off(self, zone):
        if str(zone) in self._states['zoneList'] and len(self._states['zoneList']) > 1:
            ...
            if zoneList in self._preset_modes:
                self.set_general('zoneList', zoneList)
    ...
    async def queue_commands(self):
        ...
        if not self._queueing_commands:
            ...
            for attempts in range(3):
                if attempts > 0:
                    _LOGGER.warning("Response not received, resending command: {}".format(payload))
                else:
                ...
                _LOGGER.error("No response received after 3 attempts, aborting")
    ...
    async def start(self):
        ...
        while True:
            ...
            while not self._connected:
                ...
                if attempts > 0: _LOGGER.warning("Discovery failed, retrying in {}s".format(cooloff_timer + 5))
                attempts += 1
    ...
    def server_connection_made_callback(self, transport):
        _LOGGER.info("Connected to Wifi Module")
    ...
    def server_connection_lost_callback(self):
        _LOGGER.warning("Server connection lost")
        self._connected = False
